...
bttn = InlineKeyboardMarkup(
    [[InlineKeyboardButton("🔙 Qeri gayıt", callback_data="cbmenu")]]
)
...
async def resume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "▶️ **İz davam etdirildi.**\n\n• **Yayımı dayandırmaq üçün**\ istifadə edinn» /pause əmr."
            )
        except Exception as e:
            await m.reply(f"🚫 **xəta:**\n\n`{e}`")
    else:
...
async def mute(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "🔇 **Userbot səsi söndürüldü.**\n\n• **Userbotun səsini söndürmək üçün istifadə edin**\n» /unmute əmr."
            )
        except Exception as e:
            await m.reply(f"🚫 **xəra:**\n\n`{e}`")
    else:
        await m.reply("❌ **yayımda heç nə**")
...
async def cbmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.mute_stream(chat_id)
            await query.edit_message_text(
                "🔇 userbot uğurla səssizləşdirildi", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **xəta:**\n\n`{e}`", reply_markup=bcl)
    else:
...
async def cbunmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.unmute_stream(chat_id)
            await query.edit_message_text(
                "🔊 userbot uğurla səssizdən çıxarıldı", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **xəta:**\n\n`{e}`", reply_markup=bcl)
    else:
